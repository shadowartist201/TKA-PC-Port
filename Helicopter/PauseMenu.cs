...
namespace Helicopter
{
	internal class PauseMenu : Menu
	{
		...
		public void Update(float dt, InputState currInput, ref GameState gameState)
		{
			...
			if (pause_resume.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
                gameState = GameState.PLAY;
            }
            else if (pause_options.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
                gameState = GameState.OPTIONS;
            }
            else if (pause_changeLevel.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
                gameState = GameState.STAGE_SELECT;
            }
            else if (pause_changeKitteh.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
                gameState = GameState.CAT_SELECT;
            }
            else if (pause_highscore.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
                gameState = GameState.LEADERBOARDS;
            }
            else if (pause_mainMenu.Contains(Game1.touchLocations[0].Position * Game1.resolutionDifference)) {
				Global.PlayCatSound();
