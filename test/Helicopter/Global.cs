...
public static class Global
{
	...
	public static Color[] rainbowColors = (Color[])(object)new Color[6]
	{
		Color.Red,
		Color.Orange,
		Color.Yellow,
	...
	public static Color[] rainbowColors8 = (Color[])(object)new Color[8]
	{
		Color.Red,
		Color.Orange,
		Color.Yellow,
	...
	public static void setPixel(GraphicsDevice graphicsDevice)
	{
		...
		pixel = new Texture2D(graphicsDevice, 1, 1, false, (SurfaceFormat)0);
	...
	{
		return (Color)(random.Next(0, 6) switch
		{
			0 => Color.Red, 
			1 => Color.Orange, 
			2 => Color.Yellow, 
	...
	{
		float num = RandomBetween(alpha, beta);
	...
	public static void PlayCatSound()
	{
		...
		{
		case 0:
			soundBank.PlayCue("cat_01");
			break;
		case 1:
			soundBank.PlayCue("cat_02");
			break;
		case 2:
			soundBank.PlayCue("cat_03");
			break;
		case 3:
			soundBank.PlayCue("cat_04");
			break;
		}
	}
	...
	public static bool CanBuyGame()
	{
		...
		{
			//SignedInGamer val = Gamer.SignedInGamers[playerIndex.Value];
			//if (val == null || !val.IsSignedInToLive || Guide.IsVisible)
			//{
			//	return false;
			//}
	...
	{
				vibratingPaused = true;
		GamePad.SetVibration(playerIndex.Value, 0f, 0f);
	}

	public static void SetVibrationResume()
	{
						vibratingPaused = false;
		if (vibrationOn)
		{
			if (vibratingEndless)
			{
				GamePad.SetVibration(playerIndex.Value, 0.3f, 0.3f);
			}
			if (vibratingTemp)
			{
	...
	{
		if (vibrationOn && vibratingTemp && !vibratingPaused)
		{
			vibrationTimer += dt;
			if (vibrationTimer > vibrationTime)
			{
				SetVibrationTemp(on: false);
			}
		}
	}
}
